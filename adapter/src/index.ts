 ...
 */
export const models = [
  // Anthropic — Opus 4.x (newest first)
  ...
  // Anthropic — 3.x long-tail (still useful for cost-sensitive jobs)
  ...
  // Ollama — Mistral
  { id: "mistral-large", label: "Mistral Large (Ollama)" },
  { id: "mistral-nemo", label: "Mistral Nemo (Ollama)" },
  { id: "mistral", label: "Mistral 7B (Ollama)" },
  // Ollama — Qwen
  { id: "qwen2.5", label: "Qwen 2.5 (Ollama)" },
  { id: "qwen2.5-coder", label: "Qwen 2.5 Coder (Ollama)" },
  // Ollama — DeepSeek
  { id: "deepseek-coder-v2", label: "DeepSeek Coder V2 (Ollama)" },
  { id: "deepseek-r1", label: "DeepSeek R1 (Ollama)" },
  // Ollama — Microsoft / Google
  { id: "phi3.5", label: "Phi 3.5 (Ollama)" },
  { id: "gemma3", label: "Gemma 3 (Ollama)" },
];
...
- amplifier-agent is not installed on the host (run \`uv tool install git+https://github.com/microsoft/amplifier-agent\` first; require >=0.5.0 if your agent uses sub-session delegation)
...
- amplifier-agent >=0.4.0 required (host_config layer + G3 fail-fast on headless approval ambiguity)
- amplifier-agent >=0.5.0 recommended for any agent that delegates work to sub-agents (PR #38 in microsoft/amplifier-agent fixed approval/display capability inheritance in spawn.py; earlier versions auto-deny tool calls in spawned sub-sessions)
- amplifier-agent-ts wrapper >=0.6.1 (closes 9 of 10 audited surface gaps; protocol 0.3.0)
