...
/**
 * Ensure each desired paperclip skill is materialized as REAL files inside
 * the adapter's managed skills dir. The engine's tool-skills module discovers
 * skills by scanning directories listed in `host_config.skills.skills`, and
 * its security check rejects symlinks whose targets resolve outside the
 * configured skills dir. So we recursively copy each skill into the managed
 * dir instead of symlinking — every entry the engine sees is a real file
 * with the materialized dir as its root.
 *
 * Trade-off: a few MB of duplicated content per (instance × company) and a
 * one-time copy on the first turn. Subsequent turns no-op on already-current
 * copies via mtime check.
 *
 * Why not symlink (the original choice): codex-local symlinks because the
 * codex engine doesn't enforce a boundary check. The amplifier engine does
 * (see `Skipping symlink that escapes skill directory boundary` warnings in
 * the stderr stream). Matches the claude-local materialization pattern.
 */
async function ensureAmplifierSkillsMaterialized(
  ...
  for (const entry of filtered) {
    ...
    try {
      // If the target is currently a symlink (from a prior adapter version),
      // remove it before copying to avoid following the dangling link.
      try {
        const lst = await fs.lstat(target);
        if (lst.isSymbolicLink()) await fs.unlink(target);
      } catch {
        // ENOENT — fine, nothing to clean up.
      }
      const upToDate = await targetIsUpToDate(entry.source, target);
      if (upToDate) continue;
      // Recursive copy. `force: true` overwrites existing files; `errorOnExist`
      // defaults to false. This produces real files (not links) which the
      // engine's skill boundary check accepts.
      ...
      await onLog(
        "stdout",
        `[paperclip] Materialized amplifier-local skill "${entry.runtimeName}" into ${skillsDir}\n`,
      );
    } catch (err) {
...
/**
 * Cheap freshness check — compare directory mtimes. If the target's mtime is
 * greater than or equal to the source's, we assume the materialized copy is
 * current and skip the re-copy. This avoids a full recursive re-copy on every
 * heartbeat. A change to any file inside the source tree updates the source
 * dir's mtime via `touch -m` semantics on most filesystems, but to be safe we
 * also compare top-level file counts.
 */
async function targetIsUpToDate(source: string, target: string): Promise<boolean> {
  try {
    const [src, tgt] = await Promise.all([fs.stat(source), fs.stat(target)]);
    if (!src.isDirectory() || !tgt.isDirectory()) return false;
    if (tgt.mtimeMs < src.mtimeMs) return false;
    return true;
  } catch {
    return false;
  }
  ...
  const runAttempt = async (
    ...
    try {
      handle = await spawnAgent({
        ...
        cwd,
        env: {
          // DEFAULT_ALLOWLIST (PATH, HOME, USER, TERM, LANG, TMPDIR, ...)
          // ensures the engine subprocess inherits the host shell's PATH so
          // it can find uv, git, and any other binaries it needs at runtime
          // (e.g. to fetch loop-streaming or other on-demand engine modules).
          // `extra` layers our PAPERCLIP_* + provider keys on top.
          allowlist: DEFAULT_ALLOWLIST as readonly string[] as string[],
          extra: env,
        },
    ...
    for await (const event of handle.submit(prompt)) {
      switch (event.type) {
        ...
        case "notification": {
          ...
          if (method === "usage") {
            // ACCUMULATE — the engine emits one usage event per LLM call.
            // Overwriting (the old `=` behaviour) lost everything but the
            // last call, producing 0/0 in run records for multi-call turns.
            inputTokens += asNumber(params.inputTokens, 0);
            outputTokens += asNumber(params.outputTokens, 0);
            const c = params.cost;
