...
- Provider API keys (\`ANTHROPIC_API_KEY\`, \`OPENAI_API_KEY\`, \`AZURE_OPENAI_API_KEY\`, \`AZURE_OPENAI_ENDPOINT\`, \`GEMINI_API_KEY\`, \`OLLAMA_HOST\`) can be configured per-agent via the \`env\` field (e.g. {"env": {"ANTHROPIC_API_KEY": "sk-..."}}) OR inherited from the paperclip server's host environment. Per-agent \`env\` wins on collision. Inheritance is whitelisted to known provider vars — unrelated host env vars are never inherited. Each inherited key emits a "Inherited <KEY> from host environment" line in the run log for provenance.
`;
