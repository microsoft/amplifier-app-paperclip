...
- Provider API keys (e.g. \`ANTHROPIC_API_KEY\`, \`OPENAI_API_KEY\`, \`AZURE_OPENAI_API_KEY\`, \`AZURE_OPENAI_ENDPOINT\`, \`OLLAMA_HOST\`) can be configured per-agent via the \`env\` field (e.g. {"env": {"ANTHROPIC_API_KEY": "sk-..."}}). The adapter no longer inherits provider keys from the paperclip server's host environment itself — amplifier-agent resolves its own credentials (env var, or \`~/.amplifier-agent/credentials.json\` via \`amplifier-agent auth set <provider> <key>\`) and auto-enables any provider whose credentials resolve. Per-agent \`env\` always wins on collision with amplifier-agent's own resolution.
`;
